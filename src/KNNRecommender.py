...
from src.RecommenderSystem import get_recommendation
...
def model_valuation(y_test, y_pred, pred_prob):
...
def SearchingBestModelStats(X_train, X_test, y_train, y_test):
    ...
    print("Predizioni dei primi 5 elementi: ", knn.predict(X_test)[0:5], 'Valori effettivi: ', y_test[0:5])

    y_pred = knn.predict(X_test)
    pred_prob = knn.predict_proba(X_test)

    print("Valutazione del modello...\n")
    model_valuation(y_test, y_pred, pred_prob)

    print("\nMiglioramento del modello determinando gli iper-parametri ottimali con 'Grid Search'...")
...
def main():
    ...
    recommend_data = movie_data[['title', 'release_year', 'main_genre', 'streaming_service', 'star']].iloc[movie_index]
    predict_data = movie_data[['id', 'runtime', 'imdb_score', 'tmdb_score']].iloc[movie_index]

    #80% del dataset destinato al training e 20% destinato al testing
    ...
    print("\nLista dei 5 film raccomandati, con una predizione sulla categoria star:\n", recommend_data, "\n")